...
interface WeeklyContent {
  ...
  release_date?: string; // YENİ
  is_locked: boolean;    // YENİ
  lock_reason?: string;  // YENİ
...
export default function StudentDashboard() {
  ...
  const handleWeekSelection = (weekData: WeeklyContent) => {
    // Kilitli haftaya tıklamayı engelle
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden font-roboto relative text-secondary">
          ...
          <div className="animate-in fade-in duration-500">
            {isIntroView ? (
              ...
              <div className="max-w-screen-xl mx-auto p-6 md:p-10 text-left">
                <div className="mb-10 border-b pb-8 border-gray-100 flex flex-col md:flex-row md:items-end justify-between gap-6 leading-none">
                    ...
                    {selectedWeek.materials.map((mat) => {
                      ...
                      return (
                        <button key={mat.id} onClick={() => setActiveMaterial(mat)} className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-[10px] font-black transition-all whitespace-nowrap leading-none ${activeMaterial?.id === mat.id ? 'bg-white text-primary shadow-md scale-105' : 'text-gray-500'}`}>
                          ...
                          <span className="ml-1 leading-none">{mat.title.toUpperCase()}</span>
                        </button>
                      );
                    })}
                  </div>
                  ...
                  <div className="space-y-12 animate-in slide-in-from-bottom-2">
                        ...
                        <div className="p-5 md:p-8 space-y-8 bg-gray-50/20 leading-normal">
                          {isQuizLocked() ? (
                            <div className="text-center py-12 px-6 flex flex-col items-center gap-6 animate-in zoom-in-95 leading-none">
                              <div className="w-16 h-16 bg-red-50 text-red-500 rounded-2xl flex items-center justify-center border-2 border-red-100 shadow-lg leading-none"><Lock size={32} /></div>
                    ...
                    <section className="flashcard-notes-grid space-y-12">
                      {selectedWeek.flashcards && selectedWeek.flashcards.length > 0 && (
                        <div className="bg-gray-50 p-8 rounded-[3rem] border border-gray-100 space-y-8 leading-none">
                            <div className="flex items-center justify-between leading-none text-left">
                              <div className="flex items-center gap-4 shrink-0 text-left leading-none"><div className="bg-primary p-3 rounded-2xl text-white shadow-lg leading-none text-center"><BookOpen size={24}/></div><h3 className="font-black text-secondary uppercase text-lg tracking-tighter leading-none text-left">ÖĞRENİM KARTLARI</h3></div>
                      ...
                      <div className="bg-white rounded-[3rem] border-2 border-gray-50 shadow-xl overflow-hidden flex flex-col min-h-[400px] leading-normal">
                ...
                ) : <div className="bg-gray-50 rounded-[3rem] p-24 text-center border-4 border-dashed border-gray-100 text-gray-300 font-black uppercase tracking-widest italic flex flex-col items-center gap-5 leading-none"><Eye size={64} className="opacity-10" /> LÜTFEN BİR MATERYAL SEÇİNİZ</div>}
              </div>
            )}
          </div>
        ) : (
        ...
        {isChatOpen && (
          <div className="w-[240px] md:w-[280px] h-[360px] bg-white rounded-[1.25rem] shadow-[0_15px_40px_rgba(0,0,0,0.2)] border border-gray-100 flex flex-col overflow-hidden animate-in slide-in-from-bottom-5 duration-300 ring-1 ring-black/5 shrink-0 leading-none">
            <div className="bg-secondary p-3 flex items-center justify-between text-white shrink-0 shadow-lg leading-none">
              <div className="flex items-center gap-2 leading-none"><div className="bg-primary p-1.5 rounded-lg shadow-md text-center leading-none"><Bot size={14} /></div><h4 className="text-[9px] font-black tracking-widest uppercase leading-none">BÜ-AI ASİSTAN</h4></div>
              <button onClick={() => setIsChatOpen(false)} className="hover:text-primary transition-all shrink-0 p-1 leading-none"><X size={14} /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-3 space-y-2.5 bg-gray-50/50 custom-scrollbar shrink-0 leading-relaxed text-left">
              ...
              {isTyping && <div className="flex justify-start leading-none"><div className="bg-white p-2 rounded-lg rounded-tl-none shadow-sm flex gap-1 animate-pulse border border-gray-100 leading-none"><span className="w-1 h-1 bg-gray-300 rounded-full"></span><span className="w-1 h-1 bg-gray-300 rounded-full"></span><span className="w-1 h-1 bg-gray-300 rounded-full"></span></div></div>}
              <div ref={chatEndRef} />
            </div>
      ...
      {isAnalysisModalOpen && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-secondary/90 backdrop-blur-md animate-in fade-in duration-300 overflow-y-auto leading-none">
          <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-[0_40px_100px_rgba(0,0,0,0.5)] overflow-hidden border-4 border-white my-auto flex flex-col max-h-[90vh] leading-none">
            <div className="bg-secondary p-8 flex items-center justify-between text-white border-b-4 border-primary shrink-0 leading-none">
              <div className="flex items-center gap-5 leading-none">
                ...
                <div className="leading-none text-left"><h3 className="font-black uppercase tracking-tighter text-xl leading-none">BÜ-AI Performans Raporu</h3><p className="text-primary text-[10px] font-bold uppercase tracking-widest mt-1 leading-none text-left">Yapay Zeka Eğitim Mentörü</p></div>
              </div>
            ...
            <div className="p-8 md:p-12 overflow-y-auto custom-scrollbar flex-1 bg-gray-50/50 min-h-[300px] leading-relaxed text-left">
              {isAnalysisLoading ? (
                <div className="flex flex-col items-center justify-center py-20 gap-8 leading-none text-center"><div className="w-20 h-20 border-4 border-primary border-t-transparent rounded-full animate-spin leading-none"></div><p className="text-secondary font-black text-xl uppercase tracking-widest animate-pulse leading-none text-center">Veriler Analiz Ediliyor...</p></div>
              ) : (
                <div className="bg-white border-2 border-primary/10 p-8 rounded-[2.5rem] shadow-sm leading-relaxed text-left animate-in slide-in-from-bottom-4">
                  <div className="flex items-center gap-3 mb-6 text-primary leading-none text-left"><Sparkles size={20} /><span className="font-black text-xs uppercase tracking-widest leading-none">Akıllı Geri Bildirim</span></div>
            ...
            <div className="p-8 bg-white border-t flex justify-center shrink-0 leading-none"><button onClick={() => setIsAnalysisModalOpen(false)} className="w-full md:w-auto bg-secondary text-white px-16 py-5 rounded-[2rem] font-black text-xs tracking-widest shadow-xl uppercase active:scale-95 transition-all leading-none text-center">Kapat Ve Devam Et</button></div>
          </div>
        </div>
      )}
    </div>
